...
namespace NameSpace_AFM_Project
{
    public partial class MainWindow : Form
    {
        ...
        public double[] MAX_RANGE_AXIS_NM = { SCANNER_RANGE_Z_NM, SCANNER_RANGE_X_NM, SCANNER_RANGE_Y_NM };
        ...
        void LoadAFMParamter()
        {
            ...
            if (out_data.Length == NumberOfParameters)
            {
                ...
                UpdateGUITextBox_Invoke(ref para_Dx, textBox_Dx);//, 1, SCANNER_RANGE_X_NM);
                UpdateGUITextBox_Invoke(ref para_Dy, textBox_Dy);//, 1, SCANNER_RANGE_Y_NM);
                UpdateGUITextBox_Invoke(ref para_XL, textBox_XL);//, 1, SCANNER_RANGE_X_NM);
                UpdateGUITextBox_Invoke(ref para_YL, textBox_YL);//, 1, SCANNER_RANGE_Y_NM);
        ...
        void on_Received_Package_SystemState(byte[] com_buffer, int ind)
        {
            ...
            // 1TMCU,2Tz,3TXy,4Sy,5,Sx,6PRC,7Sz
        ...
        void on_Received_Package_Indent(byte[] com_buffer, int ind)
        {
            ...
            //v_feedforward *= SCANNER_RANGE_Z_NM / BIT32MAX;
            //realtime information of the system
        ...
        void on_Received_Package_Approach(byte[] com_buffer, int ind)
        {
            ...
            if (done == 0)// continue to walk
            {
                ...
                if (mApproach_CoarseStepCounter > 10000.0 * 1000.0 / (SCANNER_RANGE_Z_NM / 2.0))// exceed max coarse steps
                {
                    Appraoch_cancel();
            ...
            if (done == 1)// finished
            {
                ...
                double STEP_SIZE_APPROACHING_nm = STEP_SIZE_APPROACHING_dac_value * SCANNER_RANGE_Z_NM / BIT18MAX;
                // double max_steps = 2521;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //serialPort_Arduino.DtrEnable = true;// on programming usb serial will reset mcu
            mCCoarsePositioner.MoveDistance(mCaxis_z, SCANNER_RANGE_Z_NM * 1.21, 1000);// move away up for safety reason
            Thread.Sleep(1000);
        ...
        {
            send_CMD_PC2MCU(CMD_PC2MCU.CMD_PC2MCU_WAVE_TEST, 50);
        }
        ...
        private void button_Update_ParaPID_Click(object sender, EventArgs e)
        {
